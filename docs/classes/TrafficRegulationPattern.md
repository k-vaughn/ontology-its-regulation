![Draft for review only](https://isotc204.org/assets/img/draft_for_review.svg)

# The Traffic Regulation Pattern of the Traffic Regulation Ontology

This ontology defines concepts related to traffic regulations.

This pattern imports the following files:

- [https://w3id.org/itsdata/regulation/v1/ConditionPattern](https://w3id.org/itsdata/regulation/v1/ConditionPattern)
- [https://w3id.org/itsdata/regulation/v1/CorePattern](https://w3id.org/itsdata/regulation/v1/CorePattern)

This pattern consists of the following classes:

- [Activation Status Code](ActivationStatusCode.md)
- [Coded Pair Regulation](CodedPairRegulation.md)
- [Coded Regulation](CodedRegulation.md)
- [Coded Regulation With Fee](CodedRegulationWithFee.md)
- [Coded Regulation With Threshold](CodedRegulationWithThreshold.md)
- [Code Pair Regulation Code](CodePairRegulationCode.md)
- [Context Regulation Code](ContextRegulationCode.md)
- [Fee Regulation Code](FeeRegulationCode.md)
- [Reason Code](ReasonCode.md)
- [Regulation Code](RegulationCode.md)
- [Regulation Code List](RegulationCodeList.md)
- [Regulatory Scope Code](RegulatoryScopeCode.md)
- [Rule Maker Role](RuleMakerRole.md)
- [Stand Alone Regulation Code](StandAloneRegulationCode.md)
- [Textual Regulation](TextualRegulation.md)
- [Threshold Regulation Code](ThresholdRegulationCode.md)
- [Traffic Regulation](TrafficRegulation.md)
- [Traffic Regulation Order](TrafficRegulationOrder.md)
- [Tro Status Code](TroStatusCode.md)
This module defines the following properties:

- [associatedTrafficControlDevice](../properties/associatedTrafficControlDevice.md)
- [authorizingTrafficRegulation](../properties/authorizingTrafficRegulation.md)
- [hasActivationStatus](../properties/hasActivationStatus.md)
- [hasAssociatedRestriction](../properties/hasAssociatedRestriction.md)
- [hasPermitType](../properties/hasPermitType.md)
- [issuedTro](../properties/issuedTro.md)
- [memberTrafficRegulation](../properties/memberTrafficRegulation.md)
- [reasonCode](../properties/reasonCode.md)
- [regulatoryScope](../properties/regulatoryScope.md)
- [troStatus](../properties/troStatus.md)


The formal definition of this pattern is available in TURTLE Syntax in two files, the [core semantics](../TrafficRegulationPattern.ttl) and the SHACL [restrictions](../TrafficRegulationSHACL.ttl).
